...
{
#ifdef ARDUINO
    // EMERGENCY FIX: Disable memory debugging to prevent watchdog timeouts
    // The get_free_max_block() call causes heap walking which triggers watchdog
    ...
    // Use simple placeholder instead
    _logOutput->printf(PSTR("[  MEM/  MEM  0]"));
#endif
}
...
{
#if LV_MEM_CUSTOM == 0
    // EMERGENCY FIX: Disable LVGL memory monitoring to prevent watchdog timeouts
    // lv_mem_monitor() can also cause delays during heap analysis
    // lv_mem_monitor_t mem_mon;
    // lv_mem_monitor(&mem_mon);

    // Use simple placeholder instead
    _logOutput->printf(PSTR("[  LVG/  LVG  0]"));
